...
class MockDatabase {
    ...
    return [
      {
        ...
        passwordHash: '5e884898da28047151d0e56f8dc62927e8acf17f25eac6f0d9e2be6f43e5c8b7', // "password123" hashed
      ...
      },
      {
        ...
        passwordHash: 'a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3', // "123456" hashed
  ...
  async verifyPassword(phoneOrEmail, password) {
    const user = this.findUser(phoneOrEmail);
    
    if (!user) {
      return { success: false, message: 'ไม่พบบัญชีผู้ใช้', user: null };
    }

    // Simulate password check (ใช้ simple hash, ใน production ใช้ bcrypt)
    const encoder = new TextEncoder();
    const data = encoder.encode(password);
    const hashBuffer = await crypto.subtle.digest('SHA-256', data);
    ...
    if (passwordHash === user.passwordHash) {
      return { success: true, message: 'รหัสผ่านถูกต้อง', user };
    }
  ...
  findUser(phoneOrEmail) {
    ...
    return this.users.find(user => {
      ...
      const normalizedInput = normalized.replace(/\D/g, '');
      return normalizedInput === phone || user.email.toLowerCase() === normalized;
    });
  }
